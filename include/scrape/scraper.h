...
void Scrape_expr_data_init (struct Scrape_expr_data *inpt, size_t nexpr);
void Scrape_expr_data_cleanup (struct Scrape_expr_data *inpt);
int Scrape_html (const char *site, struct HtmlData *data);
int Scrape_eval_expr (struct HtmlData *rawhtml, struct Scrape_instr exprs[],
		      size_t nexpr, struct Scrape_expr_data *out);
int Scrape_website (char const *site, struct Scrape_instr instr[], size_t nexpr,
		    struct Scrape_expr_data *out);
int Scrape_proccess (char const *website, struct Scrape_instr instr[],
		     size_t nexpr);
